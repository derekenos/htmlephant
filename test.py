...
def test_multiple_attrs():
    assertYields(
        Anchor(href="http://example.com", id="some-id"),
...
def test_nonvoid_with_text():
    assertYields(
        Anchor("sample text", href="http://example.com"),
        """<a href="http://example.com">
  sample text
</a>
""",
...
body > div {
  padding: 8px;
                ...
                Body(children=[Div("some text", id="id-1", _class="class-1")]),
            ],
        ),
        """<html lang="en">
  <head>
...
def test_nonempty_document():
    assertEqual(
        "".join(
            Document(
                body_els=(Span("abcd"),), head_els=(Script('console.log("hello")'),)
            )
        ),
        """<!DOCTYPE html>
<html lang="en">
