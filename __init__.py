...
class HTMLElement:
    ...
    def __init__(self, text=None, children=None, **attrs):
        ...
        if not hasattr(self, "TAG_NAME"):
            raise AssertionError(
                "Please subclass HTMLElement and define a " "TAG_NAME attribute"
            )
        # Assert that text and children are None for a void-type element.
        if self.IS_VOID and (text or children is not None):
            raise AssertionError(
                'text and children are prohibited for void tag "{}"'.format(
                    self.TAG_NAME
                )
            )
        ...
        if any(k not in attrs for k in self.REQUIRED_ATTRS):
            raise AssertionError(
                'missing required attrs {} for tag "{}"'.format(
                    [k for k in self.REQUIRED_ATTRS if k not in attrs], self.TAG_NAME
                )
            )
    ...
    def html(self, indent=0):
        ...
        # Yield the text.
        if self.text:
            text = self.escape_text(self.text) if self.ESCAPE_TEXT else self.text
